...
const faqs = [
  ...
  { question: "4. How do I set up Smootv using Xtream Codes API?", answer: "Choose Xtream Codes API in your player and enter the server URL, username, and password from your welcome email." },
...
export default function DeviceSetupGuide() {
  ...
  return (
    <section className="mb-16 md:mb-24 pt-28">
      ...
      {/* Guide Content Card */}
      <article className="mx-auto max-w-[1000px] rounded-3xl border border-amber-500/20 bg-[#0B0D14] p-6 sm:p-10 shadow-2xl overflow-hidden">
        
        {/* Device Header */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 border-b border-white/10 pb-6 mb-8">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-500/10 border border-amber-500/25 text-amber-400 shrink-0 shadow-[0_0_12px_rgba(245,158,11,0.15)]">
        ...
        {/* Important Note */}
        {device.note && (
          <div className="mb-8 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 flex items-start gap-4 text-amber-200">
        ...
        <div className="space-y-6">
          ...
          <SetupStep number="2" title="Configure Playlist">
            Launch the app, open its Settings menu, and enter the M3U URL or Xtream Codes credentials supplied in your welcome email after ordering your <Link href="/pricing" className="text-amber-400 hover:underline font-bold">Smootv subscription</Link>.
            <div className="mt-4">
          ...
          <SetupStep number="3" title="Start Watching">
            Save settings and allow a few seconds for the EPG guide and <Link href="/channels" className="text-amber-400 hover:underline font-bold">50,000+ live channel list</Link> to populate. Enjoy 4K HDR live television streaming!
          </SetupStep>
        </div>
      </article>
      ...
      <div className="mx-auto mt-20 max-w-[1000px] px-4">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {faqs.map((faq) => (
            <div className="smootv-card p-6 rounded-xl space-y-2" key={faq.question}>
...
function SetupStep({ children, number, title }: { children: React.ReactNode; number: string; title: string }) {
  return (
    <div className="flex flex-col sm:flex-row gap-6 bg-[#07080C] border border-white/5 p-6 sm:p-8 rounded-2xl">
      <div className="flex flex-1 gap-5 items-start">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 text-sm font-black text-black shadow-md">
          {number}
        </span>
